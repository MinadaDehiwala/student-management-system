...
echo "[2/4] Ensuring database exists for root local-dev mode..."
...
if [[ "${root_setup_ok}" == "true" ]]; then
  ...
  echo "Root setup unavailable. Falling back to root schema apply..."
  ./scripts/mac_apply_schema.sh
fi

echo "[4/4] Verifying app DB connection..."
mysql -h 127.0.0.1 -u root -D campus_db -e "SELECT COUNT(*) AS students FROM students;"
